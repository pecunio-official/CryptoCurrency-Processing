	...
	function cryptobox_update_page(data, ext)
	{
		...
		if (data.boxtype == 'paymentbox') $('.'+ext+'boxlogo').attr('src', 'https://coins.ctrlholdings.com/images/'+data.coinname.toLowerCase()+'/payment'+logoext+'.png'); else $('.'+ext+'boxlogo').attr('src', 'https://coins.ctrlholdings.com/images/'+data.coinname.toLowerCase()+'/captcha'+logoext+'.png');
		...
		// Raw Data froim ctrlholdings.com 
		var html = "";
	...
	function cryptobox_show(boxID, coinName, public_key, amount, amountUSD, period, language, iframeID, userID, userFormat, orderID, cookieName, webdev_key, hash, width, height)
	{
		...
		else if (userFormat == 'COOKIE' && cryptobox_cookie(cookieName) == '') { if (document.getElementById(iframeID).src != null) document.getElementById(iframeID).src = 'https://ctrlholdings.com/images/crypto_cookies.png'; alert('Please enable Cookies in your Browser !'); }
		...
		{
			var url = 'https://coins.ctrlholdings.com' + 
